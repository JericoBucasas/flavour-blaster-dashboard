-- Expose Shopify-compatible total-sales components to the server-only dashboard.
-- Shipping is derived from the authoritative current total so historical rows
-- remain reconcilable even when the original ingestion did not persist it.

alter table public.fb_orders
  add column if not exists return_fees numeric(16,2) not null default 0;
...
comment on function public.fb_dashboard_snapshot_v2(date, date) is
  'Server-only multi-store dashboard payload with Shopify-compatible total-sales components.';
